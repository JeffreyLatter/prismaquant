...
\cite{autoround2023}, HALO/HQQ-style rotation preprocessors, and most
...
selects from $\{$BF16, MXFP8, NVFP4, MXFP4, NVINT3, $\dots\}$, subject to
...
production \textsc{PrismaQuant}~v1 stack is a tuned per-tensor pipeline
that combines several state-of-the-art techniques per Linear, with a
per-layer sensitivity allocator on top. It is worth describing in its own
right, both because PrismaSCOUT inherits it wholesale (only the selection
routine changed in the head-to-head comparison of
...
\paragraph{The per-tensor toolkit.} For every quantizable Linear,
v1 composes:

\begin{itemize}[leftmargin=*,itemsep=2pt]
  \item \textbf{HALO rotation preprocessing} \cite{prismaquant} ---
    a Hadamard-style orthogonal rotation applied to the weight matrix
    before integer quantization, which spreads outlier-channel mass
    over the rotated basis and reduces the per-element representation
    range. Particularly effective for $4$-bit weight-only formats whose
    error is dominated by tail outliers.
  ...
  \item \textbf{FP8 source passthrough} (commits \texttt{705fbcd},
    \texttt{0bf4146}) --- for natively-FP8 source checkpoints, the
    allocator may pick a verbatim copy of the source FP8 codes plus
    \texttt{weight\_scale\_inv}; the BF16 view is a lossless dequant, so
    the per-Linear $\Delta\mathrm{loss}$ is exactly zero. This is
    important for native-FP8 architectures (MiniMax, DeepSeek-class).
...
The allocator on top is HAWQ-style per-layer sensitivity \cite{hawqv3}
fed by a multi-chunk empirical-Fisher probe, with a multi-choice knapsack
DP under the bit budget. Activation residency, fused-sibling coherence,
shape constraints (e.g.\ MXFP8 tile alignment, FlashInfer kernel masks),
and joint-input-global-scale handling for fused attention are all
enforced inside the DP. The whole pipeline runs end-to-end on a UMA
host with bounded memory.

\paragraph{Why this works as well as it does.} Per-layer quantization
error is, on most Linears in a modern transformer, well-modeled by a
local quadratic $\tfrac12 \langle H_i, (W_i - \hat W_i)^2 \rangle$. Each
component above attacks one term of this quadratic: HALO reduces
$\|W_i - \hat W_i\|$ by spreading outliers; GPTQ minimizes
$\langle H_i, (W_i - \hat W_i)^2 \rangle$ exactly under a fixed format;
scale sweep tunes the post-rotation per-tensor scale to minimize the
remaining error; block-output match closes the gap to the
\emph{block-level} reference, absorbing some of the inter-Linear
coupling without an explicit cross-layer cost model. The allocator's
HAWQ-style sensitivity then spends bits on the Linears whose local
quadratic has the largest top-eigenvalue. Each per-layer technique is
narrow, principled, and composable; together they extract most of the
per-Linear quantization headroom.
...
\paragraph{The stacking-interference failure mode.} The conventional
intuition for per-tensor techniques is that they are \emph{additive} in
single-format quantization: HALO reduces outliers, GPTQ minimizes the
local quadratic, scale-sweep tunes the post-rotation per-tensor scale,
block-output-match closes the residual block-level gap, and stacking
them composes the gains. In a fixed-format setting (e.g.\ all
NVFP4) this composition is empirically robust on the per-Linear
metrics each pass optimizes against. In a mixed-precision setting it is
not. Each pass operates on the activation distribution as modified by
the previous pass, and the per-Linear cost models the passes consume
are decoupled from the format choices that the allocator will eventually
make for upstream Linears. The result is that pairs of individually
validated wins can interfere catastrophically when stacked.

The canonical empirical incident is recorded in the quality-wins audit
on Qwen3-0.6B
(handover \texttt{.claude/codex-handover-2026-04-29-pm.md}). Each of
four wins (\texttt{PRISMAQUANT\_ACT\_CLIP\_QUANTILE}=$0.999$,
\texttt{PRISMAQUANT\_GPTQ\_DAMP\_SWEEP}, \texttt{PRISMAQUANT\_BLOCK\_OUTPUT\_MATCH},
\texttt{PRISMAQUANT\_DO\_NO\_HARM}) was validated individually on the
\textsc{PrismaQuant} validator EVAL\_PROMPTS suite. \texttt{solo\_clip}
alone moved PPL from $15.453$ (baseline) to $14.546$ ($-0.907$); the
other three were neutral or near-neutral on Qwen3. The cumulative
\texttt{full\_v2} configuration with all four wins enabled landed at PPL
$14.987$, $-0.466$ from baseline --- materially \emph{worse} than
\texttt{solo\_clip} alone. Roughly half the activation-clip benefit was
lost. The mechanism, root-caused after the audit: the do-no-harm gate
compares per-Linear MSE on \emph{unclipped} cached activations while
GPTQ optimized that Linear's weight under \emph{clipped} activations.
The post-pass weight is a better fit on the clipped activation
distribution but appears worse on the unclipped one; the gate reverts
clip-improved Linears back to RTN, undoing the upstream gain. This is
exactly the AWQ-style interference pattern, and it is structural.
Mixed-precision quantization sits on a directed acyclic graph of
optimizations whose intermediate states are not propagated forward
through the rest of the pipeline.

Several related incidents shaped the same lesson, all of them
quantization-pipeline issues rather than orthogonal bugs. \textsc{AWQ}
\cite{awq2024} was defaulted off in v1 after empirical bakeoffs showed
it \emph{worsened} NVFP4 group quantization on most Linears, despite
being a strict win on uniform-format $4$-bit settings (handover
\texttt{.claude/prismaquant-handover-2026-04-29.md}, line 38). The
HALO integration in \texttt{materialize\_tensors\_streaming} hung on
Qwen-3.5/3.6 multimodal models at \texttt{apply\_halo\_to\_head} when
tied embeddings collided with the \texttt{model.language\_model.*}
body prefix; HALO was force-disabled in the launcher for those
architectures (same handover, ``HALO Risks To Handle First''). HALO's
topology coverage is also fragile: it handles standard
\texttt{self\_attn.\{q,k,v,o\}\_proj} and dense
\texttt{mlp.\{gate,up,down\}\_proj}, but Qwen-3.5's
\texttt{linear\_attn.*} layers and DSv4's
\texttt{wq\_a/wq\_b/wkv/wo\_a/wo\_b} plus hyper-connections plus
packed routed experts produce silently incorrect rotations on the
unsupported sites unless the profile declares per-architecture block
specs. Onboarding Gemma~4 26B-A4B as a second base architecture
(commit \texttt{7cd0d39}, ``framework: harden onboarding via Gemma 4
smoke-out'') surfaced seven distinct framework issues that were
Qwen-biased assumptions in \texttt{sensitivity\_probe},
\texttt{export\_native\_compressed}, and the profile base class:
\texttt{text\_config} key promotion for AutoConfig resolution, a
plain-content calibration fallback for tokenizers without a chat
template, an \texttt{on\_disk\_expert\_qname} indirection,
profile-driven source-passthrough prefixes for vision/audio towers,
profile-gated MTP materialization, fused-sibling auto-fill from vLLM's
\texttt{packed\_modules\_mapping}, and a per-format toggle for whether
the exporter emits 3D packed-MoE tensors or per-expert 2D splits.
Even after those fixes, Gemma~4 served at $27$ tok/s but generated
\emph{empty text}: a Gemma-specific expert-shard routing bug had BF16
3D packed tensors landing in the wrong \texttt{w13\_weight} /
\texttt{w2\_weight} slots, an export-side packing fault that no
per-Linear cost model would have flagged because each individual
Linear's quantization was correct. The per-Linear ``do no harm'' gate
(commit \texttt{ed9c60c}) was added as a band-aid for the
stacking-interference family: after GPTQ + scale-sweep, compare
against pure RTN on the cached activations and revert if RTN beats the
post-pass weight. The export-cache fingerprint fix (commit
\texttt{7a3628e}) closes a related hazard: a per-layer export cache
hit would otherwise bypass HALO rotation \emph{and} quantization,
producing artifacts that disagreed with the recipe.
...
\begin{description}[leftmargin=2em,style=nextline]
  ...
  \item[$L_2$ perturbed-X fixed point ($\sim$\,minutes).] A more careful
    ...
    convergence is declared when the weighted-Hamming flip ratio between
    successive assignments falls below \texttt{convergence-frac} (default
    $0.01$). Inter-layer coupling enters through the activation cache:
...
\begin{proposition}[Monotone non-regression of coord-descent polish]\label{prop:monotone}
Fix a calibration prefix set $C$, a deterministic logits evaluator $\mathcal{E}$
(model class, kernel selection, RNG state, and quantization rounding mode all
held constant), and a fused-sibling-coherent move set $\mathcal{M}$. Write
$K(\Assignment) \;\equiv\; \kl(\Assignment;\,C,\,\mathcal{E})$ for the KL
measured against this fixed pair. Let $\Assignment^{(0)}$ be the chosen knee
assignment with $\kappa_0 = K(\Assignment^{(0)})$. The coord-descent polish
iterates single-refinement-unit flips
$\Assignment^{(t+1)} = \Assignment^{(t)}\!\big[i \to f^\star\big]$ drawn from
$\mathcal{M}$ and accepts only flips that strictly satisfy
$K(\Assignment^{(t+1)}) < K(\Assignment^{(t)})$. The output assignment
$\Assignment^{(\infty)}$ satisfies $K(\Assignment^{(\infty)}) \le \kappa_0$.
\end{proposition}

\begin{proof}
Each accepted flip strictly decreases $K$ on the fixed $(C, \mathcal{E})$
pair; rejected flips leave the assignment and $K$ unchanged. The sequence
$\{K(\Assignment^{(t)})\}$ is monotone non-increasing.
\end{proof}
...
(ii) a different evaluator (e.g.\ post-export inference engine, alternate
kernel choice), or (iii) post-polish assignment mutation outside
$\mathcal{M}$ (e.g.\ exporter-side fused-sibling rewrites, format coercions
imposed by serving constraints). The shipped artifact is therefore
re-validated end-to-end after export to bound the residual divergence
between the polish-time evaluator and the production inference engine.
\end{remark}
...
\texttt{scratch/deliberation/} of the source repository for future audit.
