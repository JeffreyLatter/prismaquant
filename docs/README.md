...
|---|---|---|
| `design/model_coverage_ledgers.md` | The coverage-ledger frame: four closure checks (disk bytes, read bytes per token, executed matmuls to routes, cost-model mass) whose denominators come from the checkpoint and a traced forward, never from the pipeline's own enumeration. Adopted after the `wo_a` finding (17.9% of DSv4 decode traffic was never an allocator decision). Gates are campaign R5; until they land this is the frame, not an enforced contract. | CURRENT |
