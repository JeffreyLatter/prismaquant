...
| `audits/serve_env_census_setproctitle_2026-08-14.md` | Why the serve environment census refused every correct server: it reads `/proc/<pid>/environ`, and vLLM's EngineCore renames itself via `setproctitle`, which on Linux overwrites the argv+envp block and destroys that file while `os.environ` stays intact. Structural on every lane at every commit, and never once green. Fixed with `SPT_NOENV=1` in both runtime Docker vectors. | CURRENT — read §5 before assuming the fix unblocks the DSv4-Flash 0731 bytes; it does not |
