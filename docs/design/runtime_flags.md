...
| `VALIDATED_FRONTIER_PICK` | `kneedle`, or `budget` when `TARGET_DISK_GB` is set | Selection rule for `SELECTION_MODE=validated-surrogate`: `budget` (min measured KL among the rows whose exact exported footprint fits `--target-disk-gb`; re-vet R1), `kneedle`, `best-kl`, `lowest-bpp`, `practical-knee`, `saturation`. Kneedle is axis-dependent on a log-linear RD curve — under a card, `budget` is the ship rule and kneedle is the diagnostic. |
