...
| `AQUA` | `0` | Merges an activation-quantization cost term into the weight-only cost table between stage `[2]` and the allocator (stage `[2e/4]`) — "AURA + AQUA", the recipe RobTand's shipped Qwen3.8-27B PrismaAQUA HF model card lists. `0` (default) is exactly today's weight-only-cost behavior. When `1`: builds a Sensitivity Card from `probe.pkl` (`prismaquant/sensitivity_card_build.py` → `${WORK_DIR}/artifacts/aqua_card.npz`), then merges the activation term for `AQUA_SERVING_LANE` into `COST_PATH` (`prismaquant/aqua_activation_cost.py` → `${WORK_DIR}/artifacts/cost_aqua.pkl`), and points the allocator at the merged table instead of the weight-only one. `AQUA_SERVING_LANE` (default `compressed_tensors`) must match the activation-quantization contract the export actually ships (`prismaquant/lane_specs/*.json` `"id"`). `AQUA_RENDER_BASIS` (default `rtn`) selects the card's render basis. `AQUA_ACT_DIR` (default `${WORK_DIR}/act`) supplies measured activation variance when present; falls back to the card's modeled variance otherwise. Still a research promotion — no served KL/PPL A/B against the weight-only arm at matched bpp is in-tree yet (see `docs/ARCHITECTURE.md`'s AQUA section). |
| `CLUSTER_NODES` | unset | Space-separated SSH-reachable addresses (use the fast-NIC address, e.g. a ConnectX-7/RoCE IP, not a management/VPN hostname — this rsyncs the model and dispatches the render over it) of additional boxes to render stage `[4/4]`'s production cache on. Empty/unset is exactly today's single-box behavior. When set, `prismaquant/lib/cluster_render.sh`'s `build_prod_cache_maybe_clustered` splits the render into `1 + len(CLUSTER_NODES)` shards (`PRISMAQUANT_UNIT_SHARD=i/N`, `unit_sharding.py`) — this box is always shard 0 — after a mandatory cross-box determinism preflight (`tools/cluster_render_sentinel.py`), then merges with `tools/merge_unit_shards.py`. Requires `MODEL_PATH`/`WORK_DIR` to be absolute paths and every listed node to already have the repo's `.venv` provisioned (`uv sync`) — the script fails with the exact one-line fix rather than provisioning it. v1 covers only the two `build_production_cache` call sites on the default `SELECTION_MODE=surrogate` path (the plain raw-cache build, and the combined build+recache call, which decomposes into a sharded render followed by the existing separate `production_recache` step only when clustering — solo-box keeps its one-call optimization unchanged); the `validated-surrogate` frontier cache and the two cost-side caches are not yet cluster-aware. |
...
| `TARGET_BITS` | `knee` | Allocator bit budget over quantizable parameters. `knee` (default): `run-pipeline.sh` runs the allocator's Pareto sweep once (throwaway `--target-bits`, the first `PARETO_TARGETS` entry), reads `pareto.knees.json`'s primary knee's `target_bits`, and re-runs the allocator at that value — the best measured quality-vs-compression tradeoff on this model's curve, not a fixed number. `knee-ask` does the same sweep but prints the knee vs `4.75` and prompts via `/dev/tty` before switching, refusing outright (`exit 2`) with no interactive terminal attached (e.g. under `nohup`) rather than hanging a background run. An explicit numeric value (e.g. `4.75`) always pins that exact budget and skips the sweep-and-resolve step, same precedent as `TARGET_PROFILE`. Conflicts with `TARGET_DISK_GB` (refused at startup) since a byte budget already picks its own ship point among the rungs that fit. |
