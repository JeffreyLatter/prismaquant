...
| `design/runtime_flags.md` | The `PRISMAQUANT_*` / `COST_MODE` / `SELECTION_MODE` / lever vocabulary and defaults; the most accurate defaults record in the tree. | CURRENT — known drift: `PRISMAQUANT_CB_LADDER_INTERP` is described as unwired but is read at `measure_quant_cost.py:1369` and `expert_empirical_cost.py:868`; the flag index lists 111 of 122 live names; `PRISMAQUANT_BLOCK_OUTPUT_MATCH` is default-ON (`export_native_compressed.py:5617`) with no table row; no table for the CB lane's shell vars |
| `design/progressive_render_pipeline.md` | The local render-mechanism contract: baseline/candidate/score/accept loop, declared ordering, per-format mechanism matrix (`render_score.py:214-374`, `production_weight_cache.py:1349-1381`). | CURRENT — omits the CB `weighted_vq` branch (`production_weight_cache.py:1376-1380`) and the GGUF fall-through to registry-RTN (`:1381`) |
...
| Path | What it is | Status |
|---|---|---|
| `lanes/nvfp4-cb/STANDARDS.md` | The lane's normative format/kernel/cost contract: rung ladders, scale coding, kernel standards, the split-aware floored RD cost law, the served-A/B rule. The most accurate lane doc; cited by `layer_config.py:32` and `plugins/gridbook/pyproject.toml:10`. | CURRENT — one drift: it says fp4 scale-coding v1 is "never produced by new exports", but the pipeline default is still v1 (`run-pipeline.sh:1547`) and only the shipped drivers override to `two_tier` |
| `lanes/nvfp4-cb/LAYOUT.md` | The on-disk byte/container contract a third-party plugin implements against: `type_size = 4k+16` fp4 / `4k` fp8, superblock 256, `layout_version: 2`, codebook sidecars. The public spec. | CURRENT — line 27 still lists the retired step-4 set `FP8_CB_K36/40/44/48`; code accepts every integer K28–K48 (`layer_config.py:38`) |
| `lanes/nvfp4-cb/two-tier-scale-spec.md` | The v2 scale-coding spec: per-256 E8M0 super + per-16 4-bit sub composing exactly onto E4M3; halves fp4 scale cost 0.500 → 0.28125 bpw. | CURRENT — header still reads "Spec only — nothing here is implemented"; it is implemented (`nvfp4_cb_formats.py:1290-1392`), exported (`export_nvfp4_cb.py:653-756`) and shipped in Hy3 and Laguna |
...
| `lanes/nvfp4-cb/format-speed-policy.md` | Measured deliberation: when to take CB's prefill tax vs native NVFP4 at matched bpw, and the accuracy-first policy that came out of it. Its §3.2 per-(format, shape-regime) serving-cost axis is declared future work — no λ term exists in `allocator_solver.py`. | CURRENT |
| `lanes/nvfp4-cb/PLAN.md` | The original master plan plus the lane's full running implementation log through the Hy3 ship. | HISTORICAL — header says "DRAFT … Nothing implemented"; four artifacts and a PyPI plugin shipped. Superseded as guidance by `STANDARDS.md`; keep as the provenance trail |
| `lanes/nvfp4-cb/moe_cb_design.md` | MoE CB design: route-flip cost argument, empirical-expert-cost integration, and the then-open serving gap. | HISTORICAL — §3 and §4 landed (`gridbook/moe.py`, `gridbook/moe_toplevel_loader.py`, `run-pipeline.sh:331-334`); the plugin path it names (`plugins/vllm_prismaquant/`) is now `plugins/gridbook/`. §2's route-flip argument survives |
| `lanes/nvfp4-cb/serving-kernel.md` | The original kernel/plugin plan: INV-1 (no resident dense `[N,K]`), INV-2 (tensor cores, not bf16 MMA), and the prototype ladder Triton → transient expand → fused. | HISTORICAL — the plan executed; the shipped surface is `STANDARDS.md` + `plugins/gridbook/README.md`. Kept because INV-1/INV-2 are still live vocabulary in the kernel sources |
...
| `results/production_render_staged_27b_results_2026-05-21.md` | Staged production-render allocator on 27B: improved the last-token-KL screen (0.0232 vs 0.0280) and regressed direct WikiText PPL (10.83 vs 8.33). "Do not ship". | HISTORICAL — the canonical citation for "a narrow KL screen can invert against direct PPL" |
...
| `results/qwen36_35b_mse_promotion_phase1_2026-05-25.md` | Phase-1 local-output-MSE promotion on 35B: removed 86% of stored local MSE, landed KL 0.0898 / PPL 9.81 — beat the strategic baseline, lost to both the shipped 4.75 and the 5.16 kneedle. | HISTORICAL — superseded by AURA-on-MoE; the `MSE_PROMOTION` machinery survives but defaults off (`run-pipeline.sh:211-217`) |
...
| `plugins/gridbook/README.md` | The gridbook vLLM plugin's front page: invariants, scope, registration, kernel set. The lane's most-read file. **STALE** — Scope says mixed containers raise `NotImplementedError` (delegation is implemented, `gridbook/config.py:349-374`), omits MoE entirely though three of four shipped artifacts are MoE, and calls even-split product mode the only supported form |
...
| `README.md` (repo root) | Public README: AURA framing, served results, quickstart for the compressed-tensors and GGUF lanes, format/architecture/artifact tables. **STALE** — the CB/gridbook lane is absent entirely and the subtitle's "no custom kernels" is true only of the compressed-tensors lane; the Architectures list omits Laguna |
...
| `archive/` (repo root, 18 walls) | The graveyard: one dated directory per rejected method, each with a `README.md` banner stating why it lost. **Load-bearing — never move.** Four walls are cited by path in `run-pipeline.sh` `exit 2` messages (`grouped_kl_2026-05-28`, `hdq_2026-05-14`, `fisher_2026-05-15`, `multi_shot_2026-05-19`). Convention is dated directory name + top-level banner; `prismaclip_2026-05-14` and `reap_2026-05-15` are missing banners, `entmoot` and `minimax_m2p7` are missing dates |
