...
upgraded to use the same per-Linear quantized weights the exporter ultimately
ships --- joint-NVFP4 sibling-coherent input global scales, GPTQ
reconstruction, scale-sweep, and calibrated activation clip --- and the
move set is restructured to start from a low-bpp floor with a small
bits-budget creep, drives the held-out KL further down to $0.0054$ at
$5.39$~bpp, roughly $2.8\times$ lower KL than the shipped PrismaSCOUT
artifact at comparable bpp. We also document, in a separate methodology section, several
...
Section~\ref{subsec:prodpolish}, which evaluates candidate flips on the same
quantized weights the exporter will ship rather than on RTN proxies and
sweeps from a low-bpp floor under a small bits-budget creep --- drives the
same model further to $5.39$~bpp / KL $0.0054$, $\sim\!2.8\times$ lower KL
than the shipped 5.31 artifact at $+0.08$~bpp.
...
one. \textsc{AWQ} \cite{awq2024} surfaces the same family from a
different angle: it was defaulted off in v1 after empirical bakeoffs
showed it \emph{worsened} NVFP4 group quantization on most Linears,
despite being a strict win on uniform-format $4$-bit settings. The
per-Linear ``do no harm'' gate (commit \texttt{ed9c60c}) is the
...
need to evaluate, \texttt{ProductionWeightCache} pre-renders the
\emph{exact} weight the exporter would ship: GPTQ, scale-sweep, joint
...
\texttt{max\_passes} to $100$+ would in principle close the gap, but the
top-down direction also has no convergence guarantee \emph{toward a knee};
it converges toward a budget-feasible all-NVFP4 floor that the bottom-up
sweep already produces directly. We retired the top-down direction; the
shipping production-faithful polish runs only the bottom-up sweep from the
floor.
...
not to clone at all per trial, which is the \texttt{WeightSession}
delta-quantize path (Section~\ref{subsec:prodpolish}). We mention the
...
place per trial, weights are rendered through the full export pipeline
(GPTQ, scale-sweep, joint sibling-coherent NVFP4 input global scales,
calibrated activation clip), and the move set is a single bottom-up sweep
from a low-bpp floor under a $5\%$ bits-budget creep. Table~\ref{tab:27b_prodpolish}
reports the held-out KL outcome on Qwen3.6-27B against the same calibration
that drove the upstream PrismaSCOUT selection.

\begin{table}[h]
\centering
\caption{Qwen3.6-27B: production-faithful polish vs.\ shipped
  PrismaSCOUT. Both runs use identical source weights and identical
  per-tensor toolkit; the polish operates on the same Block-CLADO decision
  units that PrismaSCOUT's surrogate hierarchy used. Held-out KL on
  disjoint validation split, identical $8\!\times\!512$ token budget.}
...
Production-faithful polish (5.39)       & \textbf{5.39}  & \textbf{0.0054} & $-2.8\times$ at $+0.08$ bpp \\
\bottomrule
\end{tabular}
\end{table}
...
(Table~\ref{tab:27b_prodpolish_diff}): $148$ of $497$ language-domain
...
\caption{Per-Linear format disagreement on the $497$-entry language-domain
  ...
  polish (5.39~bpp). Disagreement total: $148/497 = 29.8\%$.}
...
\paragraph{An incident: undercounted decision units.} A near-miss is
worth recording. The first production-faithful polish run on 27B reported
\texttt{final\_bpp} of $2.15$, which is well below the all-NVFP4 floor of
$5.07$. The cause was a key mismatch in an ad-hoc payload writer: the
serialized decision units carried their member Linears under a
\texttt{member\_qnames} key, but
\texttt{prismaquant.block\_clado.parse\_payload} only recognized
\texttt{members} and silently fell back to using the unit name as a sole
pseudo-member. Half the fused-sibling decision space (every \texttt{q/k/v}
group, every \texttt{gate\_up/down} pair) became invisible to polish, so
flips on those units were silently no-ops, and bits accounting computed
from \texttt{member\_qnames} undercounted by $\sim 2.4\times$. The fix has
two parts: \texttt{parse\_payload} now accepts both keys, and
\texttt{coord\_descent\_polish} raises a hard error if any decision unit
has zero \texttt{member\_qnames} present in the starting assignment. The
defensive assertion would have caught the bug at the first KL measurement
of the first pass; the canonical writer
\texttt{units\_and\_pairs\_to\_payload} has emitted both keys all along.
We mention the incident because it is a useful illustration of a more
general pipeline-discipline principle: cost-model and weight-rendering
fidelity is necessary but not sufficient. Move-set fidelity --- the polish
must actually be able to flip every quantizable Linear it claims it can
--- is independently load-bearing. A silent fallback that turns expensive
flips into no-ops looks indistinguishable from a polish that converged
fast.
...
polish-time evaluator is upgraded to render exactly the per-Linear
quantized weights the exporter ships and the move set is restructured
around a low-bpp floor, drops held-out KL another $2.8\times$ at $+0.08$
bpp ($5.39$~bpp / KL $0.0054$), reclaiming the rank-projection bit budget
that distributed-saliency surrogates over-protect and concentrating it on
a handful of \texttt{mlp.down\_proj} bottleneck Linears.
