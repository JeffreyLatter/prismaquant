"""The DSv4 native gate executes only from the artifact-build snapshot."""
...
def test_serve_driver_has_snapshot_reexec_and_repeated_closure_proofs():
    ...
    assert 'exec bash "$PQ_RUNTIME_SNAPSHOT/scripts/serve_dsv4_cb_validate.sh"' in source
    ...
    assert "export PQ_RUNTIME_PRISMAQUANT_ROOT=$PQ_RUNTIME_SNAPSHOT" in source
...
def test_serve_driver_refuses_dirty_bootstrap_before_snapshot_or_docker(tmp_path):
    ...
    assert "serve checkout must be clean at artifact build commit" in result.stderr
