...
| `PRISMAQUANT_ACT_CLIP_SOLVER_PREWRITE_BASELINE` | **on** | Writes each eligible group's baseline NVFP4 rendering while PrismaClip scores it, so groups that keep the baseline do not render the same weights again during final cache fill. |
