...
integer quadratic programming approach of \cite{deng2023iqp}, the
cooperative-game CoopQ allocator \cite{coopq}, and the AutoML-style AMQ
\cite{amq} --- choose how many bits to spend where. The two axes are
largely orthogonal: an allocator can sit on top of any per-tensor toolkit
and decide which Linears get 16, 8, 4, 3, or 2 bits.
...
to model the bias --- structured pairwise integer programming
\cite{deng2023iqp}, second-order ILP in HAWQ-V3 \cite{hawqv3}, Shapley-based
cooperative-game allocation in CoopQ \cite{coopq}. We take a different tack.
...
constraints. The integer-quadratic-programming approach of Deng et al.\
\cite{deng2023iqp} extends to a structured pairwise cost across layers
within a single optimization. Recent allocation work for LLMs includes
...
\cite[\texttt{scratch/powwow/02\_gemini.md}]{prismaquant} is a
...
\midrule
Held-out KL ($\downarrow$, $8\times 512$ tokens)  & 0.0151    & 0.0475    \\
Validator perplexity ($\downarrow$)               & 4.128     & 4.178     \\
Validator mean NLL ($\downarrow$)                 & 1.4178    & 1.4297    \\
Validator max prompt-avg NLL ($\downarrow$)       & 1.8946    & 1.8350    \\
GSM8K strict ($\uparrow$)                          & 96.66\%   & --        \\
IFEval prompt strict ($\uparrow$)                 & 85.40\%   & --        \\
MMLU 5-shot ($\uparrow$)                          & 86.23\%   & --        \\
\midrule
\multicolumn{3}{l}{\emph{tool-eval-bench, full hardmode suite (single trial, 148 pts)}} \\
\quad final score ($\uparrow$)                     & 88/100    & 85/100    \\
\quad total points ($\uparrow$)                    & 130/148   & 126/148   \\
\quad safety-critical failures ($\downarrow$)      & 4         & 3         \\
\midrule
\multicolumn{3}{l}{\emph{tool-eval-bench, targeted adversarial subset (10 scenarios $\times$ 3 trials, 20 pts)}} \\
\quad mean final score ($\uparrow$)                & 30/100    & 25/100    \\
\quad mean points ($\uparrow$)                     & 6.0/20    & 5.0/20    \\
\quad pass@3 ($\uparrow$)                          & 10\%      & 20\%      \\
\quad safety warnings/trial ($\downarrow$)         & 4         & 3         \\
\bottomrule
\end{tabular}
\end{table}

The 5.31~bpp artifact wins on perplexity, mean NLL, GSM8K, and aggregate
tool-eval scores, but the targeted adversarial 3-trial subset surfaces
regressions that the full single-trial run does not isolate cleanly.
Specifically, scenario \texttt{TC-42} (extra parameter injection in tool
calls) is a stable regression on 5.31 (FAIL across all three trials)
where 5.5 stably PASSES, and \texttt{TC-50} similarly degrades from PASS
to PARTIAL. The shipped 5.5 artifact and the new 5.31 artifact share a
vulnerability to \texttt{TC-60} (cross-turn sleeper injection); both
follow an attacker BCC inserted via a tool result. The validator
\emph{max prompt-avg NLL} is also worse on 5.31 ($1.8946$ vs.\ $1.8350$),
indicating that the lower mean KL is accompanied by a heavier tail.
We do not claim KL improvement implies uniform task improvement. Mean KL
hides tail regressions and underweights low-frequency schema and
tool-call tokens. Future work includes p95/p99 KL diagnostics and a
schema-focused KL gate (Section~\ref{sec:discussion}).
...
\bibitem{deng2023iqp} Z.~Deng, S.~Sharify, X.~Wang, and M.~Orshansky.
  Mixed-Precision Quantization for Deep Vision Models with Integer
  Quadratic Programming. \emph{arXiv:2307.05657}, 2023.
