...
| `PRISMAQUANT_FP8_GPTQ_BLOCK_SIZE` | `32` | Column block size for FP8_E4M3/FP8_E5M2 GPTQ error propagation. Lower values are cheaper; larger values may improve local reconstruction at higher cost. |
| `PRISMAQUANT_MXFP8_JOINT_SCALE_SHIFTS` | `-4,-3,-2,-1,0,1,2` | Candidate E8M0 exponent shifts searched by MXFP8_E4M3/MXFP8_E5M2 GPTQ when `joint_scale_opt` is enabled. This is a true legal-scale search around the conservative no-saturation exponent, not NVFP4 max-to-4/max-to-6 scaling. |
| `PRISMAQUANT_MXFP8_SCALE_SWEEP_SHIFTS` | `0` | Legacy standalone candidate E8M0 exponent shifts for MXFP8_E4M3 activation-weighted scale search. Nonzero shifts are experimental; .8B and 4B A/Bs on 2026-05-10 regressed KL, so production defaults to RTN-equivalent MXFP8_E4M3 outside the GPTQ+joint-scale path. |
...
| `PRODUCTION_CACHE_LEVERS` | `gptq,joint_scale_opt` | V1 production render levers: GPTQ with damp sweep plus joint scale optimization. NVFP4 uses its established fused-sibling/global-scale path; FP8_E4M3/FP8_E5M2 use GPTQ; MXFP8_E4M3/MXFP8_E5M2 use GPTQ and, when `joint_scale_opt` is enabled, legal E8M0 block-scale search. `scale_sweep` remains available for explicit ablations but is no longer the default. |
