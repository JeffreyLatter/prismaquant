...
| Path | What it is | Status |
|---|---|---|
| `lanes/nvfp4-cb/STANDARDS.md` | The lane's normative format/kernel/cost contract: rung ladders, scale coding, kernel standards, the split-aware floored RD cost law, the served-A/B rule. The most accurate lane doc; cited by `layer_config.py:32` and `plugins/gridbook/pyproject.toml:10`. | CURRENT — production exports default to two-tier/v2; v1 is explicit legacy read compatibility |
...
| `lanes/nvfp4-cb/format-speed-policy.md` | Historical measured deliberation about CB/native speed. | SUPERSEDED AS POLICY — raw measurements remain evidence; production selection uses hard, phase-specific byte/TTFT/ITL/TPS/memory/legality constraints, not `quality + λ·time` or blanket decode neutrality |
