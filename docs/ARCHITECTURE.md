...
As of: 2026-07-30 · commit `8fc15a2` · branch `claude/docs-consolidation` (= `main` + NVFP4-CB lane)
...
token-disjoint construction are documented at `validate_assignments_kl.py:505,573`. Small-scale
...
flowchart TD
  ...
  NOSMOKE["no in-lane serving smoke<br/>stated at :1650-1654"]
  OUTGG --> GGSMOKE
  OUTCB --> NOSMOKE
...
|---|---|---|---|---|---|
| **1/4** | Sensitivity probe — per-Linear empirical Fisher `h_trace`, body + MTP in one pass | `prismaquant.incremental_probe` (`544-560`) | `artifacts/probe.pkl`; activations → `act/`; shards → `work/`; `logs/probe.log` | settings-hash (`539`); reuse also re-checks stored `calibration_modality` (`568-598`) | — |
...
| **4/4 E-cb** | CB col-weights + codebook export | col-weights (`1556-1582`), `export_nvfp4_cb[_streaming]` (`1592-1641`) | `exported_nvfp4_cb/` | **bare skip** on col-weights (`1556`); export always runs | CB lane; no in-lane serving smoke (`1650-1654`); **exits 0 at `1661`** |
...
`validated-surrogate` are fully wired but **opt-in** (`314-336`, `825-956`, and `1092-1313`
...
2. **Two of the three approved owner names have no implementation.** `StreamingActivationCache`
   ...
   tree; the real streaming owner is `layer_streaming.LayerCache` (`layer_streaming.py:1028`).
...
`0.5 · h_trace · weight_mse · gain` (`allocator_solver.py:34-37`, derivation
`allocator.py:13-52`).

**MoE normalization.** Each expert's `h_trace` is divided by its **routed token count**
(per-routed-token mean). `route_prob` is metadata only; the second `1/p_e` division was removed
in audit M4 (`sensitivity_probe.py:36-46`, `h_detail_version: 3` at `:477`). Docs saying
"divided by routing probability" are stale.
...
PPL −24.4% (`run-pipeline.sh:186-200`). 27B-class confirmation is ladder debt.
...
`PRISMAQUANT_COST_UCB_Z` adds `z·stderr` before the DP (`allocator_candidates.py:281-307`);
...
`kl_ucb = mean + z·stderr` over calib repeats (`validate_assignments_kl.py:632-651`), consumed
...
activation-cache/model-loading utility (`validate_assignments_kl.py:40-45`,
`kl_measurement.py:50`, `production_recache.py:27`).

**L3 propagated is opt-in and OFF.** The penalty applies only when
`--propagated-sensitivity-report` is passed (`allocator.py:1310-1334`) and the pipeline leaves
it empty (`run-pipeline.sh:218`).

### 4.5 Solver

`allocator_solver.py`. Multi-choice knapsack DP over average-bits-per-parameter bins,
numpy-vectorized (`solve_allocation :219-312`); the baseline per Linear is its cheapest
candidate, bins = `(target − min_bits)/bit_precision + 2`, and backtrack mirrors the forward
charge exactly. `_charged_bins` (`:201-216`) charges any strictly positive Δbits at least one
bin, so sub-half-bin upgrades are never free.

**Serving-unit promotion is union-find.** `promote_serving_units` (`:112-129`) unions
fused-sibling and packed-MoE groups in one order-independent pass; `_promote_group_components`
(`:68-109`) gives each component the max-rank format; `promote_fused` (`:162-198`) hard-asserts
post-promotion coherence. `solve_with_promotion` (`:374-429`) loops solve → promote → measure
achieved, tightening the target by half of any overshoot (`max_iters=40`,
`overshoot_tolerance=0.01`). The invariants this exists to protect, and their failure modes,
are §6.4.

Candidate legality, passthrough integrity, cost-source precedence and fused-sibling aggregation
live in `allocator_candidates.py`; the invariants they enforce are also §6.4's.
...
(`allocator.py:216-227`); raw-linear and global-log knees are diagnostics.
`_rd_curve_diagnostic` (`:279-332`) fits `log10(Δloss)` vs bpp and at `R² ≥ 0.99` prints that
...
`worst_rank_inversion`, leave-one-out kneedle stability (`:400-479`).

**Byte-budget "fit the card"** is a third path, CLI-only and **not wired into
`run-pipeline.sh`**: `allocator.py --target-disk-gb` (`:1049`, impl `:1984-2120`) takes the
...
`saturation_select.select_under_byte_budget` (`:164`), then ratchet-bisects the sub-second DP
for an exact fit. This is the answer when the RD diagnostic says the kneedle is axis-dependent.
§12 D12.
...
`scale_count_for_shape` (`:86-118`) handles `scale_block_shape`, per-channel `group_size==0`,
and 3-D packed-expert stacks; `memory_bytes_for_shape` / `effective_bits_for_shape`
(`:120-129`) are what the DP, `footprint.py`, and the Pareto table consume. Aliases:
`FP8`/`FP8_DYNAMIC` → `FP8_E4M3`, `MXFP8` → `MXFP8_E4M3` (`:133-143`).

| Format | line | w-bits / group / scale | eff. bpp (2-D) | Status |
|---|---|---|---|---|
| `NVFP4` | `:630-640` | 4 / 16 / fp8 e4m3, A4 g16 | 4.5 | Production, in the default menu (§3.3) |
| `FP8_E4M3` | `:713-725` | 8 / per-channel / fp32, A8 per-token | 8 + 32/in_f | Production (`FP8_DYNAMIC`), in the default menu |
| `BF16` | `:761-771` | 16 / — | 16 | Production, **passthrough only**, in the default menu |
| `FP8_SOURCE` | `:788-800` | 8 / block (128,128) / fp32 | 8.00195 | Production, **passthrough only**, verbatim copy |
| `MXFP8_E4M3` | `:683-691` | 8 / 32 / e8m0 | 8.25 | Registered, profile-allowed, **de-menued** |
| `NVFP4A16`, `MXFP4`, `MXFP6_E3M2/E2M3`, `MXFP8A16`, `MXFP8_E5M2`, `FP8_E5M2`, `INT8_W8A16`, `INT4_W4A16_g128` | `:641-758` | — | — | Research / registry-only |
| GGUF k-quants + IQ | `:847-865` | `_make_gguf_spec :827` | 2.0625–8.5 | GGUF lane (§9.3) |
| `NVFP4_CB_K12..K24` / `_S13..S16` / `FP8_CB_K28..K48` | `:943-948` | VQ codebook, g256 | 2.000–3.500 / 2.125–2.500 / 3.5–6.0 | gridbook CB lane (§9.2) |
...
`FP8_SOURCE`'s `quantize_dequantize` is identity (`:798`): the bf16 view *is* the lossless
...
`nvfp4_cb_footprint.py`, since one `FormatSpec` cannot model both (`:917-923`).

NVFP4 *weight* RTN routes through the export codec (`_nvfp4_export_aligned_rtn` `:599-626`) so
...
uses a static `input_global_scale` (`:599-614`). The `torch.compile` RTN hot path is
...
(`export_native_compressed.py:107-128`): `static_6` (default), `four_over_six_mse`,
...
- **JSO = `joint_mse` evaluated inside the GPTQ loop**, per-group levels default `(6.0, 4.0)`
  (`_parse_joint_scale_levels :288-306`). The full 7-level grid collapses to {6,4} for 99.998%
...
Sweep **OFF** since 2026-06-12 (`gptq_damp_sweep_enabled` `export_native_compressed.py:1845`,
env default `"0"` at `:1857`), fixed damp **1.0** (`_resolve_gptq_fixed_damp :1860`). The
...
overrides the constant, per-role overrides at `:1897-1931`. **A second reader disagrees on the
default** — `kl_sensitivity_probe.py:285` defaults the same variable to `"1"`; §12 D5.
...
`four_over_six` (40); `static_act_order` → `joint_scale_opt` → `gptq` (all 50, ordered by
before/after); `fisher_gptq` (50, archived); `scale_sweep` (60, after gptq). The production
lever set is §3.3.
...
(`export_native_compressed.py:869-888`); it rescues blocks far below calibration amax from FP8
...
`prismaquant/export_native_compressed.py` (8,413 lines) turns a `layer_config.json` recipe plus
...
`_quantize_2d` `:4042-4523`, dispatching on `_canonical_export_format` `:672`.

| Format | codec | emitted tensors |
|---|---|---|
| NVFP4 | `quantize_dequantize_nvfp4` `:2449`, packer `pack_fp4_indices` `:847` | `weight_packed`, `weight_scale` (fp8 e4m3, g16), `weight_global_scale`, `input_global_scale` (`:1523-1530`) |
| MXFP4 | `quantize_dequantize_mxfp4` `:2778` | packed fp4 + uint8 E8M0 scales (g32) |
| MXFP8_E4M3 / _E5M2 | `quantize_dequantize_mxfp8` `:2851` | fp8 weight + uint8 E8M0 scales (g32) |
| FP8_E4M3 / _E5M2 | `quantize_dequantize_fp8_dynamic` `:2998` | fp8 weight + per-row fp32 `weight_scale` |
| BF16 | `_passthrough_tensor` `:5023` | verbatim |
| FP8_SOURCE | verbatim copy (§6.3) | source `weight` + `weight_scale_inv` |
| 3-D packed experts | `_quantize_3d_packed` `:4524` + `_split_packed_expert_tensor` `:3842` | per-expert per-projection tensors |

Activation-aware passes compose inside `_quantize_2d` (`:4076-4133`): `gptq`, `scale_sweep`,
`static_act_order`, `joint_scale_opt`, the latter two forced to require `gptq`
(`:4132-4133`). `input_global_scale` follows the compressed-tensors `generate_gparam`
convention `FP8_MAX·FP4_MAX/max_abs` (`:909`, applied `:4067-4074`).

**M19 — export honours the render's scale rule.** `_export_match_render_scale_rule`
`:1432-1449` reads the cache's `levers["nvfp4_scale_rule"]` and re-derives NVFP4 codes under
*that* rule rather than the entry default `static_6`, making the re-quant of the cache's bf16
dequant near-idempotent; `PRISMAQUANT_NVFP4_EXPORT_MATCH_RENDER_SCALE` **defaults ON**. Packed
companion `_packed_expert_render_scale_rule` `:1452-1479` — without it, joint_mse-rendered
experts re-derived under `static_6` flipped 43% of packed bytes. Residual gap: joint scale
*levels* are not in the lever dict, so a non-default `PRISMAQUANT_NVFP4_JOINT_SCALE_LEVELS`
must match between cache-build and export (`:1467-1470`). `_pack_production_cached_2d`
`:1482-1583` re-packs only — no re-run of GPTQ/scale-sweep, which would measure a different
artifact (`:1491-1494`).

**Block-output match** (`prismaquant/block_output_match.py`), `PRISMAQUANT_BLOCK_OUTPUT_MATCH`
**default `"1"` = ON** (`:5464-5465`, `:5617`, `:5763`): for NVFP4 dense block Linears (q/k/v/o,
gate/up/down) it defers the pack, greedily refines per-Linear group scales against an FP16
block-reference forward, then finalises (`:5851+`). Its own ~0.05–0.10 PPL estimate predates
JSO and has never been re-measured on the gold lane (§12).

### 6.2 `config_groups` / `ignore` / packed-MoE emission

`build_quantization_config` `:6857` emits explicit per-name targets grouped by format, remapped
to vLLM-internal names via `profile.to_vllm_internal_name`; the catch-all default group is the
format with the most non-BF16 members (`:6870-6872`). Schemes are hand-authored constants —
`NVFP4_SCHEME` `:6543`, `MXFP8_SCHEME` `:6560`, `MXFP4_SCHEME` `:6578`, `FP8_SOURCE_SCHEME`
`:6601`, `FP8_E4M3_SCHEME` `:6621`.

BF16 plus `bf16_passthrough` plus `extra_ignore` go to `ignore` (`:6881-6904`); BF16 **packed**
experts additionally need a per-layer regex over every (expert, projection) because vLLM
scheme-dispatches on per-expert Linear qnames (`_bf16_packed_expert_ignore_regex` `:6682`, used
`:6900-6902`). Fused siblings present in the serving model but absent from the probe (Gemma4
`k_eq_v` with no `v_proj`) are back-filled into `ignore` from `packed_modules_mapping` / the
structure spec (`:6906-6942`). `compute_extra_ignore` `:7323` must *not* add per-expert source
keys when the packed parent is quantized — that marks the FusedMoE un-quantized, the NVFP4
scale params never register, and weight-load KeyErrors follow (`:7297-7306`).
`_preflight_quantization_config` `:7276` builds the entire config before any GPU render or
shard write, so metadata violations fail in seconds rather than hours.
...
regex pinned to that layer index** (`_constrain_per_expert_projection_regex` `:6646`,
`_pin_regex_to_layer` `:6635`, emission `:6944-7045`), with on-disk leaves translated through
`_vllm_moe_scheme_projection_names` so LFM2.5's `w1/w3/w2` are advertised as
`gate_proj/up_proj/down_proj` (`:6990-6999`).

### 6.3 FP8_SOURCE verbatim, MTP, audits

`_build_fp8_source_map` `:5043-5150`: a tensor qualifies when `<base>.weight` has a sibling
...
(`:5062-5065`). Overlay `_fp8_source_config_overlay` `:5181`.

transformers v5 does not instantiate MTP for Qwen3.5/3.6 MoE, so `_materialize_mtp_tensors`
`:8134-8186` rebuilds a standalone `MtpModule` (`prismaquant/mtp_module.py`) under a parent
named `mtp` and materialises it in memory, keeping checkpoint-convention names.
`validate_mtp_assignment_coverage` `:8380-8404` **hard-fails** when the source has `mtp.*`, the
profile `has_mtp()`, and the recipe has no `mtp.*` entries. (MTP construction bypasses the
profile — §8.5 L2.)

`_bf16_upgrade_audit` `:1286` (emitted `:7899`) classifies each BF16 Linear as
passthrough/immutable, runtime-coerced, or a genuine budget choice — a manifest, not a policy.
`_coerce_runtime_legal_assignment` `:1211` is the defensive legality re-check for stale or
hand-written recipes. `_unify_input_global_scales_across_fused_siblings` `:3926` +
`_compute_nvfp4_joint_global` `:3990` force one global scale per fused group (vLLM warns and
degrades otherwise). `_production_cache_fingerprint` `:1108` / `_production_cache_expected_keys`
`:1084` gate cache↔assignment coverage.
...
|---|---|---|
| Fused siblings (q/k/v, gate/up) share **one** format | union-find `promote_serving_units` `allocator_solver.py:112-129`; hard assert `promote_fused` `:162-198`; export re-check `:7160-7175` | ≥2 quantized schemes → load crash (merged-column scale-shape assert). Quantized + BF16 → **loads and silently corrupts**: measured 4.3× worse served KL on Qwen3.x DeltaNet `in_proj_ba` (0.106 vs 0.025 at matched bpp) |
| Packed MoE experts uniform per FusedMoE (mix across layers, never within) | same union-find pass via `profile.packed_expert_format_group`; export raise `:7047-7059` | unservable; the raise names the usual root cause — allocation produced under `DefaultProfile` because the probe lacked `meta['model']` |
| Incomplete fused groups → BF16 + `ignore` | `allocator.py:1290-1296`; ignore back-fill `:6906-6942` | the fused loader expects all siblings; a missing `v_proj` breaks the merged Linear |
| Packed `config_groups` use vLLM **canonical** scheme names | `:6990-6999` | no scheme binds to FusedMoE; `w2_input_global_scale` never registers; `load_weights` KeyError |
| Multi-format menu must not resolve to `DefaultProfile` | `validate_default_profile_format_menu` `allocator.py:955`, called `:1355-1359` | silently produces the fused-coherence bug class above |
| Final serving promotion is a no-op | `validate_final_serving_promotion_noop` `allocator.py:1004` | a late promotion means the DP priced an assignment that is not the one shipped |
| Passthrough integrity (BF16/FP8_SOURCE only if the source already is) | `allocator_candidates.py:18-21`, `:106-115` | synthesising BF16 from a dequantised FP8 source burns 8 bpp for nothing |
| Registry ↔ served metadata agree on bits/group | **not enforced** — `FormatSpec` (`format_registry.py:44-129`) and the export `*_SCHEME` constants (`:6543-6632`) are independent sources of truth with no reconciling test | a divergence mis-prices bpp or mis-declares the served scheme; §12 |
...
(`run-pipeline.sh:269`, `:1208`; option `:824`), `--n-calib-samples 32`, `--calib-seqlen 1024`,
and `--calib-skip-first $NSAMPLES` for held-out disjointness (`:1194-1219`); the CLI's own
defaults (2 × 128, `:767-925`) are not what ships. `_kl_repeat_summary` `:632-651` emits
`kl_mean/std/stderr/kl_ucb`. GPU-only via `gpu_guard.require_cuda_hot_path`. Its docstring
still says "last-token KL" — stale since M26.
...
(`:29`); defaults 65,536 wikitext tokens, 200 MMLU questions, calib 8 × 512 on split `test`
...
eager by default, `--no-enforce-eager` `:138` is the graph-mode arm and **both must pass**
(`:175`). Flashinfer pinned from the profile's `runtime_package("flashinfer")` (`:30-71`);
...
`validate_assignments_kl.py:272`, `_calibration_provenance` `:299` (calib sha256),
`assignment_hash` `:1336`/`:1372`, cache `cache_hit_count` / `rtn_fallback_count` `:363-365`.
...
architecture conditionals. An AST scan of every string literal under `prismaquant/` found 5
arch-name sites in non-comment code and 2 in `plugins/gridbook/` — all enumerated in §8.5.
...
flowchart TD
  subgraph R1["registry 1 -- model structure"]
    ...
    REGY["model_profiles/registry.py:46-57 _REGISTERED<br/>order load-bearing; DefaultProfile fallback at :167"]
  end
  ...
  CONSUMERS["consumers -- 84 detect_profile call sites in 24 modules<br/>probe, cost, cache, allocator, exporters, validators"]
  REGY --> CONSUMERS
  ...
  subgraph R2["registry 2 -- serving profiles"]
    ...
    RESOLVE["resolve_target_profile -- serving_profiles.py:414-436<br/>explicit request wins first (:426-427)"]
  end

  SPEC -->|"default_serving_profile"| RESOLVE
  SPROF --> RESOLVE
  RESOLVE --> ALLOCGATE["allocator candidate legality<br/>allocator_candidates.py + allocator.py:1256"]
  ...
  L1["LEAK 1 -- run-pipeline.sh:91<br/>TARGET_PROFILE hardcoded to vllm_packed_moe and passed<br/>unconditionally (:471, :1081); spec.default_serving_profile<br/>can never win. hy_v3 declares gguf, laguna declares nvfp4_cb."]
  L2["LEAK 2 -- mtp_module.MtpModule imported directly<br/>incremental_probe.py:2666, incremental_measure_quant_cost.py:586,<br/>export_native_compressed.py:8154. profile.build_mtp_module has<br/>one caller: model_profiles/validate.py:256. DSv4 gets a Qwen3.5 MTP."]
  ...
  L4["LEAK 4 -- hardcoded arch tests in the core stack<br/>streaming_model.py:92-99 (minimax_m2 FP8 rewrite bypass) and<br/>incremental_probe.py:113-122 (MiniMaxM2Experts class name)"]
...
| Serving constraints | `serving_profiles.py` + `serving_profile_specs/<id>.json` (schema `prismaquant.serving_profile.v1`) | per-format allow/deny rules with name conditions, shape rules, runtime shape validators, runtime package requirements; `extends` composition (`serving_profiles.py:377-411`) |
...
`Qwen3MoeProfile` before `Qwen3Profile`, commented as such). `detect_profile` (`:69-86`) keys
on `config.json` `model_type` + `architectures`; unmatched models fall to
`DefaultProfile(architectures=archs)` (`:167`). `register_profile` (`:60-66`) inserts at index
0 so third-party profiles win.

The fallback is *guarded, not silent*: `allocator.py:1355-1359` calls
`validate_default_profile_format_menu(...)`, which refuses a multi-format menu under
`DefaultProfile` unless `--allow-default-profile`, on the grounds that fused-sibling coherence
and packed-expert uniformity (§6.4) cannot be enforced without arch knowledge (rationale
`allocator.py:1343-1354`).
...
staging, layer prefixes, lm_head, probe skips), `base.py:169-723`. Deliberately Python-only,
because they are forward-pass *behaviour* rather than naming: MTP (`:242-272`, `:725-738`),
streaming-probe adapters (`:740-836` — `checkpoint_to_live_name`, `fp8_scale_pairs`,
`init_rotaries`, `expand_hidden_for_layers`, `extra_layer_kwargs`, …), cross-layer forward
state for Gemma4 KV sharing (`:838-867`), `register_vendored_modeling()` (`:888-893`).
`vllm_fused_moe_scheme_projection_names` (`:443-468`) is intentionally hardcoded to vLLM's
canonical names — §6.2.

`structure.py`'s `build_model_graph` (`:444-510`, five parallel name spaces per tensor) is a
declared contract, not an executor — `base.py:913-922`, "intentionally not called from hot
paths yet"; production reads the accessors.
...
| minimax_m2 | `minimax_m2.py:48` | ❌ **none** — 6 hand-coded overrides `:69-152` | — → `research` | ⚠ none | `has_mtp → False` `:101` |
...
which carries no format allow-list (medium; unshipped since M2.7). **`deepseek_v4.json` has no
`default_serving_profile`** (key dump: `schema, id, match, shard_regexes, naming, fused_groups,
packed_experts, moe, probe, passthrough_prefixes, pinned_names`) → `research` → any menu format
passes the serving gate (medium). **`serving_profile_specs/vllm_qwen3_5_packed_moe.json` is
dead** — an empty `extends: [vllm_packed_moe]` alias whose own description says not to use it,
and nothing references it; delete after checking shipped artifact metadata.
**Mistral-Medium-3.5-128B is in the shipped family table (§1.2) with no profile** —
`grep -i mistral` over `model_profiles/` and `serving_profile_specs/` is empty, so it ran under
`DefaultProfile`; the `allocator.py:1355` gate would refuse that menu today. Finally, **no spec
declares `unpacked_expert_projection_names`** although `base.py:470-495` documents it as
spec-overridable and `incremental_probe.py:288` consumes it — every arch rides the
...
| # | Leak | Severity |
|---|---|---|
| L1 | `run-pipeline.sh:91` hardcodes `TARGET_PROFILE:=vllm_packed_moe` and passes it unconditionally (`:471`, `:1081`); `resolve_target_profile` gives the explicit request precedence (`serving_profiles.py:426-427`), so `spec.default_serving_profile` is **never consulted through the production orchestrator**. `hy_v3.json` (`gguf`) and `laguna.json` (`nvfp4_cb`) are silently overridden. Mitigated only by the export-container gates (`run-pipeline.sh:106`, `:124`), which turn the mismatch into a hard error the operator must already know to avoid. Fix: unset the shell default. | **high** |
| L2 | MTP construction bypasses the profile. `prismaquant/mtp_module.py` is Qwen3.5-specific (imports `Qwen3_5MoeDecoderLayer`/`Qwen3_5DecoderLayer`, `:79-86`) and is imported **directly** by `incremental_probe.py:2666`, `incremental_measure_quant_cost.py:586`, `export_native_compressed.py:8154`, gated only on the arch-agnostic `profile.has_mtp()`. The declared extension points `build_mtp_module` / `load_mtp_state_dict` (`base.py:248-272`) have exactly **one caller in the tree** — `model_profiles/validate.py:256`, the offline validator. Consequence: `deepseek_v4` (`has_mtp → True`, `build_mtp_module → None`) would today be handed a Qwen3.5 MTP module. Latent export bug; blocks DSv4 ship. | **high** |
...
| L4 | Two MiniMax hardcodes in the core stack. `streaming_model.py:92-99` (`model_type.startswith("minimax_m2")` / `MiniMaxM2`) decides whether to bypass transformers-5.x's FP8 pre-load module rewrite — an architecture property, and `ModelProfile` already owns the neighbouring `fp8_scale_pairs` (`base.py:783-793`). `incremental_probe.py:113-122` (`type(module).__name__ == "MiniMaxM2Experts"`) gates a probe-speed batched-expert replay; `packed_expert_module_class_names()` (`base.py:182-192`) is the spec-driven accessor for exactly that lookup. Failure modes: wrong load path (medium) and silent loss of a speed optimization, not a correctness bug (low). | medium / low |

Cosmetic, listed so they are not re-discovered as leaks:
`export_native_compressed.py:90,147-148` imports `Qwen3_5Profile` for `_COMPAT_QWEN_PROFILE`
(verified test-only back-compat); `_fast_kernel_guard.py:86-90`'s Qwen substring list is a
labelled fallback for remote HF IDs with no local `config.json`; `layer_streaming.py:1637-1639`
...
### 8.6 The validator nobody runs

`python -m prismaquant.model_profiles.validate --model <path>` implements 8 conformance checks
(docstring `validate.py:17-53`): profile claim `:134`, vLLM class resolvable `:151`,
fused-sibling self-consistency against vLLM's own sibling lists `:189`, name-remap fixed points
`:217`, MTP module construction `:244`, source-passthrough prefixes matching ≥1 real tensor
`:268`, packed-expert param names `:299`, serving profile exists and its validator callables
import `:336`. Exit 0/1, CI-shaped — and nothing invokes it: no CI workflow, no
`run-pipeline.sh` preflight, no test. Checks 5 and 8 would have caught L2 and the
`deepseek_v4` serving-profile gap on the day each was introduced. §12 D11.
...
(`gridbook`, `pyproject.toml:8`; in-tree version `0.1.0`, `gb/__init__.py:13`; PyPI is ahead at
0.1.1), registered through vLLM's general-plugin entry point (`pyproject.toml:88-89`).
...
from `config.json` (`gb/config.py:4-10`, `export_nvfp4_cb.py:627`); the non-globbed extension
...
|---|---|---|---|
| dense decode M≤16 CUDA GEMV; prefill `cb_expand_fp8` → stock `cutlass_scaled_mm` | DEFAULT | — | `gb/linear.py:46,53,355-492` |
...
| MoE fp8-CB prefill `auto` (per-layer cuda-event selection over stock + `grouped_fused` × feasible TileM) | PROMOTED-DEFAULT, two-model gate | `3062fbf` | `gb/moe.py:396`, policy `gb/moe_autotune.py`; 35B 4,405 vs best-fixed 4,285; Laguna 2,063 vs 1,821 |
...
| R6 smem-resident decode LUT | PROMOTED-DEFAULT (compile-time, no env gate) | `1ede688` | `gb/csrc/cutlass_fork/sm120_cb_fused_mma.hpp:167-193`; k48 L1 cliff removed, −61.5% kernel-level, bit-exact 38/38 |
| single-storage dense weights (issue #1) | PROMOTED-DEFAULT, unconditional | `bf1ada0` | `gb/linear.py:266-289`; 27B weights 35.86 → 20.82 GiB, KV 1.40M → 1.76M tokens |
| `l2_pipeline` MoE prefill | **DIAGNOSTIC-ONLY** — wedged the live serve three times | `afc64ec` | `gb/moe.py:392-395`; excluded from `auto` unless `PRISMAQUANT_CB_L2_AUTOTUNE=1` |
...
| decode contract v2; w2 side-stream overlap; w2 rowpack | measured NULL/negative, opt-in | `d924d76`, `08263af` | — |

fp4-CB MoE prefill is still the per-expert loop (`gb/moe.py:396`); persistent/grouped
...
with `PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True`: `serve_qwen27b_smoke.sh` (util 0.80),
...
from the same activation cache the cost stage used (`:1554-1583`; REQUIRED for every CB target
...
the out-of-tree plugin (`run-pipeline.sh:1645-1647`). Two lane defaults do not match shipping
practice (§12 D15).
...
| artifact | result |
|---|---|
| Qwen3.6-27B @5.5 bpp | vs shipped PrismaAURA-5.5bit on the same BF16 dump: conf-KL −45…−53%, ALL-KL −56/−58%, 19.93 vs 23 GB; all 386 body Linears chose CB rungs |
...
Unresolved; check the Hub before citing either (§12 D19).
...
| PrismaClip / PrismaFisherClip | Subsumed by JSO's per-block scale grid — clipping is another way of asking what the right scale is. | `archive/prismaclip_2026-05-14/` (no banner, §12 D18) |
...
| REAP / expert pruning | Cost model under-counts token redistribution and misrouting. Hit size via format/factorization, not pruning. | `archive/reap_2026-05-15/` (no banner, §12 D18) |
| Entmoot expert-merge | Never wired into the runtime. | `archive/entmoot/` (undated dir name) |
...
| Kneedle as the ship rule | Axis-dependent and LOO-unstable (fp32 4B: elbow at 5.00 in 454/1000 bootstraps). Byte budget + saturation B* replaced it; `allocator.py:325` says so in the CLI itself. | demoted, not removed |
...
| CB `l2_pipeline` MoE prefill | Wedged live serving three times; DIAGNOSTIC-ONLY, excluded from `auto` (`gb/moe.py:391-395,1357-1369`, `afc64ec`). | same |
...
Honest register, code-cited, as of 2026-07-30 (`claude/docs-consolidation`, HEAD `8fc15a2`).
...
| D4 | **`spec.default_serving_profile` is dead** — §8.5 L1. | §8.5 L1 | HIGH | Unset the shell default; keep the env var as an override. |
| D5 | **`PRISMAQUANT_GPTQ_DAMP_SWEEP` default is split between readers**: `"0"` in the exporter, `"1"` in the KL sensitivity probe. Any A/B touching both paths compares different renders. | `export_native_compressed.py:1857` vs `kl_sensitivity_probe.py:285` | HIGH | Make the probe default `"0"`; add a test pinning the two readers together. |
...
| D7 | **`pyproject.toml` on `main` is `0.1.0` while PyPI serves `0.4.1`** from tag `v0.4.1` (`d058267`), which is **not an ancestor of `main`** — its `requires-python>=3.11`, `pillow` dep and `pytest-timeout` fixes exist only on the tag. | `pyproject.toml:7`; `git merge-base --is-ancestor v0.4.1 main` → false | HIGH | Merge or cherry-pick `d058267` onto `main` and bump the version there. |
| D8 | **Export never enforces production-cache residency.** The exporter has no `--production-cache-prefetch` argument and its prefetch helper has no `require` mode; a cache miss silently yields 0 prefetched keys (NVMe-bound export). | `run-pipeline.sh:1691-1698`; `export_native_compressed.py:7425`, `_production_cache_prefetch_assignment` `:1392-1412` | MED | Add a `require` mode mirroring `production_weight_cache.py:461-478`. |
...
| D11 | **`model_profiles/validate.py` is never run** (§8.6). 8 conformance checks, CI-friendly exit codes, zero callers; there are no workflow files anywhere in the tree. | `model_profiles/validate.py:17-53`; callers → 0; `find -name .github` → none | MED | Add a CI job (or a `run-pipeline.sh` preflight) invoking it for `MODEL_PATH`. |
| D12 | **`TARGET_DISK_GB` byte-budget selection is CLI-only.** §4.6 names the byte budget as the ship rule that replaced kneedle, but `run-pipeline.sh` has no hits — every byte-budget ship is a manual `allocator.py` invocation. | `allocator.py:1049` `--target-disk-gb`; `grep TARGET_DISK_GB prismaquant/run-pipeline.sh` → 0 | MED | Plumb `TARGET_DISK_GB` through the allocator stage. |
...
| D16 | **Block-output match ships ON with a pre-JSO justification.** `PRISMAQUANT_BLOCK_OUTPUT_MATCH` defaults `"1"`; its ~0.05–0.10 PPL estimate predates JSO and has never been re-measured on the gold lane (§6.1). | `export_native_compressed.py:5464-5465` | MED | Gold-lane A/B on one 27B-class artifact, then keep or flip. |
| D17 | **Registry and export metadata are unreconciled sources of truth** for bits/group per format — `FormatSpec` vs the `*_SCHEME` constants, with no test comparing them (§6.4, last row). | `format_registry.py:44-129`; `export_native_compressed.py:6543-6632` | MED | Add a parametrized test asserting scheme ↔ spec agreement per production format. |
| D18 | **Two dead env flags are documented as live**: `PRISMAQUANT_L2_CUDA_GRAPHS` (sole occurrence is a comment saying it is intentionally not applied) and `PRISMAQUANT_DO_NO_HARM_MIN_GAIN` (no occurrence at all; its documented default `0.0` is fiction). Four more index entries (`*_SENTINEL`, `PRISMAQUANT_GRAPH_POOL`) are Python identifiers, not env vars. | `docs/design/runtime_flags.md:45,171`; `perturbed_x_cache.py:1225` | LOW | Delete the six entries; add the ~25 undocumented gridbook serving flags. |
| D19 | **8 dead launchers reference deleted modules.** `examples/launchers/{launch-qwen3-0p6b-block-clado, iterate-block-clado, iterate-block-clado-full-of, sandwich-block-clado, validate-block-clado-kneedle, polish-block-clado-best, launch-qwen3-0p6b-output-fisher, polish-frontier}.sh` — none of `block_clado.py`, `measure_output_fisher.py`, `coord_descent_polish.py` exists in `prismaquant/`. | `ls examples/launchers/` | LOW | Delete, or move alongside `docs/archive/block_clado/`. |
| D20 | **Two archive walls have no banner README** (`archive/prismaclip_2026-05-14/`, `archive/reap_2026-05-15/`) — the latter walls off live-adjacent code (`expert_prune.py`, `allocator_prune.py`, `observers/`, 5 tests) and encodes a policy the code still enforces. Two more walls violate the dated-directory convention (`archive/entmoot/`, `archive/minimax_m2p7/`). | `ls archive/*/README.md` | LOW | Add the two banners; date the two directories. |
...
| D22 | **`gridbook` PyPI 0.1.0 token was pasted and is still un-revoked** (recorded 2026-07-29). Not a code defect; a credential-hygiene item that keeps surviving handovers. | auto-memory `gridbook_pypi_release` | LOW | Revoke. |
