...
| `PRISMAQUANT_PROBE_BATCHED_ACT_TRANSFER` | `0` (off) | Restores the v22 "Fix E1" phase-1 activation transfer: hold all L+1 layer activations device-resident, then stack them for a single device→host copy. Default (off) streams each layer's activation to host inside the forward loop, which bounds device residency to one activation instead of two full activation sets at the phase-1/2 transition where the probe's high-water mark already sits — a doubling DSv4's multi-stream (`hc_mult`-wide) hidden can't afford. The memory saving is the measured claim; the relative *transfer-time* cost of per-layer vs batched copies is **unmeasured**, and this flag exists to A/B it. Read once per probe run; both paths report their true copy time as `host transfer`. Batched mode requires every captured activation to share one shape (it stacks them). |
| `PRISMAQUANT_KV_COTANGENT` | **on** | The KV-cotangent path. On cross-layer KV-sharing architectures (Gemma4 `num_kv_shared_layers>0`) the phase-3 reverse sweep grafts grad-enabled leaves over the borrowed K/V, sums each consumer's `leaf.grad` per source, and drives the *storing* layer's backward with that sum alongside its own output cotangent (`torch.autograd.backward([out, k, v], [g_out, gk, gv])`). Without it a sharing layer's backward stops at the detached capture and the storing layer's `k_proj`/`v_proj` `h_trace` — plus the chained `grad_out` of every layer below it — is under-counted. `0` restores the pre-fix severed cotangent for an A/B, and re-arms the guard below. Verified equal to an end-to-end backward in `tests/test_kv_cotangent_path.py`. |
| `PRISMAQUANT_ALLOW_KV_SHARED_FISHER` | `0` | Probe guard override for KV-sharing architectures (MINOR-M33). Now only reachable with `PRISMAQUANT_KV_COTANGENT=0`: that severs the shared-consumer cotangent and *under*-counts the storing layer's `k_proj`/`v_proj` `h_trace`, so the probe fails fast; set `1` to probe anyway, accepting the under-count. (Earlier revisions of this table called it an aliased-Fisher *double*-count — wrong direction; the missing edge only ever removes gradient.) |
...
|---|---|---|
| `PRISMAQUANT_EXPERT_COST_SAMPLE` | `0` (off) | Stratified expert subsample for COST entries: prices `S` of `E` experts and takes the mean, at `E/S` less quantize work. **Export always quantizes every expert** — this only touches the DP's cost estimates. Applies at two granularities: (a) stacked-expert (3D) weights, where the allocator prices the whole stack as one unit so the sample estimates exactly the quantity it consumes; and (b) **unpacked per-expert 2-D Linears** (DSv4-Flash: 256 experts × 3 projections × 43 layers), where a deterministic evenly spaced sample of expert ids per `(experts-prefix, projection)` group is measured and every skipped expert's cost row is filled with its group's sampled mean — rows are extrapolated rather than omitted because `build_candidates` drops row-less names, which would silently shrink the DP's bit/disk accounting and serving-unit membership. (b) is wired into all three dense measure paths — `measure_unbatched`, `measure_batched_gpu`, and `_measure_production_render_dense` (the path taken under the default `COST_MODE=production-render-score`). Added because an exhaustive IQ grid search on a 3.5G-element stack ran ~25 min/layer at full `E`, and a full-stack NVFP4/FP8 registry quantize swap-kills a unified-memory box the same way (2026-07-11). |
