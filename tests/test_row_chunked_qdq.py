...
def test_unknown_format_falls_back_rather_than_guessing():
    ...
    # `plugin.spec` is the shared FormatSpec singleton from the format
    # registry (`format_registry.REGISTRY["NVFP4"]`), not a per-call copy —
    # mutating it here without restoring leaks a renamed "NVFP4" into every
    # later test in the process (silently breaks any `spec.name == "NVFP4"`
    # dispatch, e.g. bpp global-scale accounting). Restore in `finally`.
    orig_name, orig_family = plugin.spec.name, plugin.spec.family
    try:
        # A format the chunker does not know about must decline, so `price`
        # uses the dense reference path instead of silently rendering
        # something else.
        ...
        object.__setattr__(plugin.spec, "name", orig_name)
        object.__setattr__(plugin.spec, "family", orig_family)
