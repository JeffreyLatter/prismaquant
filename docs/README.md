...
| `design/runtime_flags.md` | The `PRISMAQUANT_*` / `COST_MODE` / `SELECTION_MODE` / lever vocabulary and defaults; the most accurate defaults record in the tree. | CURRENT — known drift: `PRISMAQUANT_CB_LADDER_INTERP` is described as unwired but is read at `measure_quant_cost.py:1369` and `expert_empirical_cost.py:868`; the flag index lists 111 of 122 live names; no table for the CB lane's shell vars |
