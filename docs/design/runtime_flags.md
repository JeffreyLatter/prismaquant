...
| `PRISMAQUANT_BLOCK_OUTPUT_MATCH` | **on** (`"1"`) | `export_native_compressed.py:5623`, `:5769`; fingerprint `:5470` | Quality lever #12 (`block_output_match.py`): after the per-Linear scale pass, each layer's attention and MLP blocks run a greedy coordinate-descent refinement of per-Linear scales against an FP16 reference **block** output, capturing inter-Linear composition (q_proj error amplified through the attention dot product) that per-Linear MSE cannot see. Requires an activation cache; keeps FP32 snapshots of the block's standard Linears (~64-128 MB/layer). `0` disables. **Default-ON and previously undocumented.** |
...
| `PRISMAQUANT_NVFP4_JOINT_SCALE_OPT` | `0` | `export_native_compressed.py:7610`; `production_weight_cache.py:2162`; `kl_sensitivity_probe.py:295` | Direct JSO switch for callers that bypass `PRODUCTION_CACHE_LEVERS`. |
...
| `PRISMAQUANT_GPTQ_DAMP_SWEEP` | **`0` in the exporter**, **`1` in `kl_sensitivity_probe`** | `export_native_compressed.py:1857`, fingerprint `:5462`; `kl_sensitivity_probe.py:285` | ⚠ **The two readers disagree.** Damp sweep is OFF by default for production export (fixed damp 1.0, 2026-06-12); the KL sensitivity probe still defaults it ON. Any A/B that touches both paths must pin the value explicitly or the arms differ silently. `1` reproduces historical artifacts. |
...
| `PRISMAQUANT_GPTQ_STATIC_ACT_ORDER` | `0` | `export_native_compressed.py:7602`; `production_weight_cache.py:2158`; `kl_sensitivity_probe.py:290` | Direct static-act-order switch for callers that bypass `PRODUCTION_CACHE_LEVERS`. |
...
| `PRISMAQUANT_FISHER_WEIGHTED_GPTQ` | **archived** | `production_weight_cache.py:2169`; `kl_sensitivity_probe.py:303` | Fisher-weighted GPTQ is archived under `archive/fisher_2026-05-15/` and rejected by the production pipeline. |
...
| `COST_MODE` | `production-render-score` (`run-pipeline.sh:187`) | `production-render-score` (default) renders the full `FORMATS` menu for every Linear through `ProductionWeightCache` and writes an allocator-compatible cost.pkl from the recorded production render scores. `production-render-staged` first builds the normal baseline cost, then renders NVFP4 for every eligible Linear, ranks Linears by post-render local forward error, renders FP8_DYNAMIC candidates only for the high-error tail, marks unmeasured promotions unavailable, and lets BF16 compete only in the staged tail. `local` keeps the legacy `h_trace × output_mse` objective (and is **required** for the GGUF and nvfp4_cb containers — see the gates at `:99` and `:121`). `aura` runs the AURA downstream-KL-adjoint cost (`aura_cost.py`, served −38% KL @4B / −17.9% @27B vs the h_trace×output_mse baseline) against a production-rendered dW cache; on packed-MoE models the route-flip-blind smooth cost is replaced for experts by measured empirical unit-KL (`expert_empirical_cost.py`, FP8 kept in the menu) merged into one hybrid payload, with MTP/visual sidecar rows backfilled from the baseline cost. AURA is fully wired but **opt-in** (sub-stages at `:314-336`, `:825-956`). `grouped-kl`, `fisher`, `hdq` and `multi-shot` are **archived** and `exit 2`. |
...
| `PRODUCTION_RENDER_COST_NSAMPLES` / `_SEQLEN` / `_SEED` | `8` / `1024` / `42` | Calibration contract for `COST_MODE=production-render-staged` and `production-render-score`, using the production cache scorer. |
...
| `PRISMAQUANT_EXTERNAL_WEIGHT_MANAGEMENT` | unset | `kl_sensitivity_probe.py:1544`; `perturbed_x_cache.py:799` / `:1099` | Declares that weights are staged by an external owner (`weight_session`); also flips the `STRICT_ASSIGNMENT_COVERAGE` default to ON. |
...
| `PRISMAQUANT_ALLOW_PYTORCH_FALLBACK` | unset | `_fast_kernel_guard.py:42` | Permit the slow PyTorch path where a fast kernel is expected. |
...
| `PRISMAQUANT_CB_LADDER_INTERP` | `0` | `measure_quant_cost.py:1369` (dense path); `expert_empirical_cost.py:868` (expert path) | **Live in both cost paths** (the older "declared wiring point, not read by any code" note here was wrong). `1` enables per-`(family,mode)` RD-law ladder interpolation: anchors + holdout are measured normally, predicted rungs are fitted per TENSOR and holdout-gated with a measured fallback, so a tensor that defies the law never receives an interpolated cost (`encode_tiers.md` §B/§C). One shell knob drives both wirings: `CB_LADDER_INTERP=1` (`run-pipeline.sh:323-324` and `run-pipeline.sh:1014-1015`). |
| `PRISMAQUANT_CB_LADDER_TOL` | `0.10` | `measure_quant_cost.py:1339` | Holdout-gate relative-error tolerance for the dense ladder; matches the expert stage's `--ladder-holdout-tol` default. |
...
- `PRISMAQUANT_GPTQ_DAMP_SWEEP` has **different defaults in the exporter and in
  `kl_sensitivity_probe`** (§2). Pin it explicitly in any A/B that runs both.
