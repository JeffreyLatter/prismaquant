...
| `COST_MODE` | `production-render-staged` | `production-render-staged` first builds the normal baseline cost, then renders NVFP4 for every eligible Linear through `ProductionWeightCache`, ranks Linears by post-render local forward error, renders FP8_DYNAMIC candidates only for the high-error tail, and writes an allocator-compatible cost.pkl from the recorded production render scores. Unmeasured promotions are marked unavailable, and BF16 only competes in the staged tail. `production-render-score` renders the full `FORMATS` menu for every Linear; `local` keeps the legacy objective; `grouped-kl` is the fused-unit KL research path. The default production menu is `NVFP4,FP8_DYNAMIC,BF16`; MXFP8/E5M2 remain explicit research/legacy formats. |
