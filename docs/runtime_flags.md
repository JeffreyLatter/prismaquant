...
| `PRISMAQUANT_NVFP4_INPUT_GSCALE_FP8_RANGE` | **on** | C1 (2026-07-02 audit): NVFP4 `input_global_scale` uses the compressed-tensors/vLLM `generate_gparam` convention `448·6/amax` (FP8_E4M3 max × FP4_E2M1 max), so serve-time FP8-stored activation block scales span the whole FP8 range. The legacy `6/amax` value was 448× below convention — activation blocks with amax ≫64× below the calibration amax got subnormal FP8 scales, and ~1024× below rounded to 0 (vLLM zeroes the block). `0` reproduces historical artifact bytes. Served A/B (35B frontier artifact, byte-identical weights, 2 window draws): **−14.1% pooled served KL**, PPL −0.09%; on a thin-calibration artifact (8-sample amax) it regressed +5.8% — `0` is legitimate only for such starved-calibration reproductions. |
