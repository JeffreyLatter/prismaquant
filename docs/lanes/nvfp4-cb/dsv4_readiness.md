...
| 5 | **Plugin FusedMoE CB method** — the `moe_cb_design.md §4` contract: load `cb_qweight (E,out,bytes)`, split fused gate_up by canonical names, INV-1/CUDA-graph clean. | `plugins/vllm_prismaquant/` (**not this agent**) | ~250–400 | serve |
| 6 | **TP-sharded CB byte-stream correctness** — prove/enforce 256-superblock-aligned RowParallel splits, or export a per-shard-legal layout; validate at TP=2. | `plugins/vllm_prismaquant/linear.py` + export | ~80–150 | serve |
