...
As of: 2026-08-14 · branch `feat/aura-sensitivity-contract` · verified against
implementation baseline commit `ecc560f` plus the Gridbook pin promotion
...
the exact Gridbook installed-import-origin closure, the strict Gridbook runtime-contract-v3
...
one-purpose CPU-only W8A16 readmission plus tracked pre-export handoff gate, and
the shareable Sensitivity Card contract with its format-cost plugin seam and the
default-ON probe per-channel Fisher marginal emission that backs it (§4.8, §8.7,
§12 D30). That last item changes exactly one shipping default, and it is
...
`git+file://<copy>@<pin.commit>` VCS target; the full requested/resolved commit is then checked in
