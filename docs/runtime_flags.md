...
| `PRISMAQUANT_RENDER_PROGRESSIVE_GATES` | **on** | Production-cache render gate for local mechanisms. All formats use the same progressive order, while unsupported mechanisms are format-gated off. NVFP4 can score FourOverSix, GPTQ, joint-scale optimization, and optional scale_sweep candidate packages; FP8_E4M3/FP8_E5M2 can score GPTQ; MXFP8_E4M3/MXFP8_E5M2 can score GPTQ using the canonical E8M0 scale rule; FP8_E4M3 and MXFP8_E4M3 can additionally score explicit scale_sweep when enabled. Regressive candidates keep the prior accepted render. Cache metadata records decisions in `render_gates`; FourOverSix has a compact `four_over_six` summary. |
...
| `PRODUCTION_CACHE_LEVERS` | `gptq,joint_scale_opt` | V1 production render levers: GPTQ with damp sweep plus joint scale optimization where the format supports it. NVFP4 uses its established fused-sibling/global-scale path; FP8_E4M3/FP8_E5M2 use GPTQ; MXFP8_E4M3/MXFP8_E5M2 use GPTQ with the canonical E8M0 scale rule and do not consume `joint_scale_opt`. `scale_sweep` remains available for explicit ablations but is no longer the default. MXFP8/FP8 activation scores use their runtime dynamic activation quantizers; NVFP4 is the only current score path that applies the calibrated activation-max clip. |
