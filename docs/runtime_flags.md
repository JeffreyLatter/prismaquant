...
| `PRISMAQUANT_FISHER_COL_WEIGHTS` | `0` | Opt-in: `aura_cost` also emits a per-Linear per-column KL-Fisher energy vector (`stats[name]['fisher_col']`, length `in_features`, sums to `h_trace`) alongside the scalar cost. Strictly additive — the rest of the cost payload is bit-identical when off. Feeds `fisher_col_weights.py` (nvfp4-cb exp 4: Fisher vs imatrix `E[x²]` codeword/scale weighting). Equivalent to `aura_cost --collect-col-energy`. |
