...
| `PRISMAQUANT_ACT_CLIP_SOLVER_RESCALING` | `none` | Candidate rescale modes for PrismaClip threshold proposals. `none,row_rms` enables **PrismaClip-RBC**: every threshold is scored both as a plain clamp and as a row-RMS-preserving clamp/rescale, with final acceptance still based on output MSE on original unclipped activations. `rbc`, `auto`, and `joint` are aliases for `none,row_rms`. |
| `PRISMAQUANT_ACT_CLIP_RBC_MAX_RESCALE` | `8.0` | Maximum row-wise amplification allowed after a PrismaClip-RBC clamp. This prevents heavily clipped rows from being re-amplified into a new pathological local objective. |
...
| `PRISMACLIP_RBC` | `0` | Pipeline switch for PrismaClip-RBC. Enables `PRISMACLIP=1` and sets `PRISMAQUANT_ACT_CLIP_SOLVER_RESCALING=none,row_rms` unless already overridden. Still emits ordinary NVFP4/MXFP8 runtime formats; only the render objective changes. |
