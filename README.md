...
- **Codebook (`NVFP4-CB` / `FP8-CB`)** — served by **gridbook**, our out-of-tree vLLM quantization plugin, which *does* ship its own Blackwell CUDA kernels. Still an unforked vLLM: install the exact commit in `prismaquant/gridbook_runtime/gridbook_runtime_pin.json`, no core patches. See below.
